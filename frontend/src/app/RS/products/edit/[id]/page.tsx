...
export default function EditProductAdmin({ params: paramsPromise }: { params: Promise<{ id: string }> }) {
    ...
    img.onload = () => {
      ...
      canvas.toBlob((blob) => {
        if (blob) {
          ...
          // Determine if it was an existing image or new file
          const existingImagesCount = formData.images.length;
          if (editingIndex >= existingImagesCount) {
             newFiles[editingIndex - existingImagesCount] = file;
          } else {
             newFiles.push(file); // Or handle existing replacement logic
          }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        alert("Jewelry masterpiece updated!");
  ...
  if (fetching) return <div className="pt-40 text-center font-premium">Opening Vault...</div>;
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-20">
      <header className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-premium font-bold tracking-tight mb-1">Edit Masterpiece</h1>
          <p className="text-[10px] uppercase tracking-widest text-foreground/40 font-bold">{formData.productId}</p>
        </div>
      </header>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white border border-gold-primary/10 p-10 shadow-sm space-y-8">
            ...
            <div className="space-y-6">
              <div className="space-y-4">
                <label className="text-[10px] uppercase tracking-widest font-bold text-foreground/40 block">Gallery</label>
                <label className="cursor-pointer flex items-center justify-center gap-2 px-6 py-4 border-2 border-dashed border-gold-primary/20 bg-gold-soft/5 text-gold-primary hover:bg-gold-soft/20 transition-all w-full">
                  <Plus size={18} />
                  <span className="text-[10px] uppercase tracking-widest font-bold">Add More Photos</span>
              ...
              <div className="relative border-t border-gold-primary/5 pt-6">
                ...
                <label className="text-[10px] uppercase tracking-widest font-bold text-foreground/40 block mb-4">Paste Image URL (New)</label>
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        ...
                        if (url) {
                          ...
                          setSelectedFiles(prev => [...prev]); // Keep length sync
                ...
                <p className="text-[8px] text-foreground/30 mt-2">Press Enter to add the URL to your gallery</p>
              </div>
                      ...
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2">
                        ...
                        <button 
                          ...
                          onClick={() => {
                            ...
                            setPreviews(newPreviews);
                            if (i < formData.images.length) {
                              const newImages = [...formData.images];
                              newImages.splice(i, 1);
                              setFormData({ ...formData, images: newImages });
                            } else {
                              const newFiles = [...selectedFiles];
                              newFiles.splice(i - formData.images.length, 1);
                              setSelectedFiles(newFiles);
                            }
                          }}
                          className="px-3 py-1.5 bg-red-500 text-white text-[8px] uppercase tracking-widest font-bold"
                        >
                          Remove
                        </button>
                      </div>
        ...
        <div className="lg:col-span-1 space-y-8">
          <div className="bg-white border border-gold-primary/10 p-10 shadow-sm space-y-8">
            ...
            >
              {loading ? "Updating..." : "Update Masterpiece"}
            </button>
          </div>
        </div>
      </form>
